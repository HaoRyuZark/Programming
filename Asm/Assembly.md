...
## Example Eucliden Algorithm

```asm 

section .data   ; input data (static)
    x dd 7
    y dd 5

section .text   ; defining the entry point
    global main
...
main:
    ...
    call gcd_loop      ; call to the actual algorithm

    hlt                ; halt execution 

gcd_loop:

    cmp rbx, 0         ; if y (rbx) is 0, we are done (looping condition)
    je done
    ...
    mov rbx, rdx       ; move remainter to x
