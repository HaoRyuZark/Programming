...
The following is a sample `tsconfig.json` file with the respective key value pairs. 
...
### Advanced Types Annotations

- **Value Union**: bounds type and possible values to a variable. 

- **Type Union**: like in python, a variable can only be of a certain family of types. 

- **Type Intersection**: it has to be a combination of two types. This is common with more complex types.
...
They work mostly like JavaScript, but with type checks and the possibility to make `readonly`.
...
Functions now allow for type Annotations, default values and even we can mark possible null values with 
`param?: type`.

We also get **function types** for anonymous functions and also **overloading**
