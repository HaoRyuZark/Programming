...
def min_recall_precision(y_true, y_pred): # own metric
...
grid = GridSearchCV(estimator=LogisticRegression(max_iter=1000), param_grid={'class_weigth':[{0:1,1:v} for v in np.linspace(1,20,30)]}, scoring={'precision':make_scorer(precision_score), 'recall_score':make_scorer(recall_score), 'min_both': make_scorer(min_recall_precision)},refit=True ,cv=10, n_jobs=-1)
...
This are post processing steps done to tune the output of a model 
"""
