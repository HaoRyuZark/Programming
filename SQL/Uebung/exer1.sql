...
SELECT id as "IDX", name Product, price "Price [€]"
FROM shop_product;

-- calculated values
SELECT id "ID ", concat(name, ' [', unit, ']') "Product", price "Price [€]", round(price / (1.0 + vat), 2) "excl. VAT", round(price / (1.0 + vat) * vat, 2) "VAT "
FROM shop_product;
...
-- select * from shop_product;
SELECT name, price FROM shop_product WHERE price = 1.99;

-- string comparison with like including wildcards % and _
SELECT name, price FROM shop_product WHERE name like '%Pizza';

SELECT name, price
FROM shop_product
WHERE
    name like '%Pizza'
    and not name like '%Spinach%';
...
SELECT brand, risk_of_loss_percent
FROM shop_customer
WHERE
    risk_of_loss_percent is not null;

-- oder by, asc, desc
SELECT name, price
FROM shop_product
WHERE
    category_id in (1, 3, 4)
ORDER BY price;
-- asc
SELECT name, price
FROM shop_product
